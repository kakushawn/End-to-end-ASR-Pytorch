...
class TCC300Dataset(Dataset):
    def __init__(self, path, split, tokenizer, bucket_size, ascending=False):
        # Setup
        # path to kaldi data. e.g. data/tcc300/{train,dev,test}
        self.path = path
        self.bucket_size = bucket_size

        # List all wave files
        for s in split:
            with open("/".join([self.path, s, 'wav.scp'])) as fp:
                lines = fp.read().splitlines()
            file_list = [Path(line.strip("\n").split(" ")[4]) for line in lines]
            # Read text
            with open("/".join([self.path, s, 'text'])) as fp:
                lines = fp.read().splitlines()
            text = ["".join(line.strip("\n").split(" ")[1:]) for line in lines]
        text = Parallel(n_jobs=-1)(delayed(tokenizer.encode)(txt) for txt in text)
        # self.text = [tokenizer.encode(txt) for txt in text]
